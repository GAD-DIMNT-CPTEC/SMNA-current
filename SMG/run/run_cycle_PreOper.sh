...
# 27 Jul 2026 - L. F. Sapucci - Versao do run_cycle Adpatada para rodar pre operacional em contar de usuario
# !REMARKS:
#
#EOP
#-----------------------------------------------------------------------------#
...
while [ ${LABELI} -le ${LABELF} ]; do
   ...
   if [ ${do_gsi} -eq 1 ] ; then
     ...
     if [ ${CODIerroGSI} -ne 0 ]; then 
       echo -e "\033[31;1m > Falha no GSI: Codigo de erro: ${CODIerroGSI} \033[m"
#        exit 1 
       echo "data:" 
       date
       GSIok=0
...
# Rodando o Bam dentro do ciclo de assimila��o  
   ...
   if [ ${CODIerroGSI} -ge 4 ] ; then
     echo ""
     echo "#################################################################################"   
     echo "Processo interrompido por problemas na rodada anterior do cilo de assimila��o"
     ...
     echo "!!!!!!!!!!!!!!!!!!!!!!" >> ${stSMNA}
     echo "Status: INTERROMPIDO " >> ${stSMNA}
     echo "!!!!!!!!!!!!!!!!!!!!!!" >> ${stSMNA}
     echo "" >> ${stSMNA}
     echo " Acionar equipe de assimila��o para reiniciar o processo na data anterior.... "  >> ${stSMNA}
     echo "" >> ${stSMNA}
     echo "#############################################################################"  >> ${stSMNA} 
     echo "Processo interrompido por problemas na rodada anterior do cilo de assimila��o" >> ${stSMNA}
     echo "Enviando aviso para os responsaveis providenciar a rodada do passo anterior ">> ${stSMNA}
     echo "#############################################################################"  >> ${stSMNA}
     echo "yes" >  /pesq/share/das/dist/luiz.sapucci/SMNAfluxCPT/sendEmail.txt
     echo "" >> ${stSMNA}
     echo " Motivo:. "  >> ${stSMNA}          
     echo "codigo de erro do GSI: Exit "${CODIerroGSI}
     if [ ${CODIerroGSI} -eq 4 ] ; then
       echo ">>>>> Falta dados do fisrt guess da rodada anterior,"  >> ${stSMNA};
       echo ">>>>> Falta dados do fisrt guess da rodada anterior," 
     fi
     if [ ${CODIerroGSI} -eq 5 ] ; then
       echo ">>>>> Executavel do GSI n�o encontrado,"  >> ${stSMNA}; 
       echo ">>>>> Executavel do GSI n�o encontrado,"
     fi
     if [ ${CODIerroGSI} -eq 6 ] ; then
       echo ">>>>> Coeficientes de BC da rodada anterior n�o encontrado."  >> ${stSMNA};
       echo ">>>>> Coeficientes de BC da rodada anterior n�o encontrado." 
     fi
     echo "Ciclo interrompido"
     exit 0
   fi

   if [ ${GSIok} -eq 1 ] ; then
   
     echo "&&& " $LABELI $Psok $radioOK  ${ContNotOk} `date`
     ...
     # Enviando email mesmo com o status ok
     echo "yes" >  /pesq/share/das/dist/luiz.sapucci/SMNAfluxCPT/sendEmail.txt
     ...
     echo " Sleep 5 min"
     echo "Tempo passado desde a ultima rodada:" $((ContNotOk*5)) "min"
     sleep 300
     
   fi

   if [ ${ContNotOk} -gt 72 ] && [ ${CODIerroGSI} -eq 2 ] ; then
     echo ""
     echo "######################################################################"   
     echo "Enviando aviso para os responsaveis providenciar os dados disponiveis"
     echo "Tempo de espera: "  $((ContNotOk*30)) "Minutos"
     echo "Que e maior que o final da janela de 6h dados usada"
     echo "######################################################################"  
     ...
     echo "Rodada das "$LABELI  >> ${stSMNA}
     echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" >> ${stSMNA}
     echo "Status: ATRASADO dados do Fluxo INPE" >> ${stSMNA}
     echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" >> ${stSMNA}
     echo "" >> ${stSMNA}
     ...
     echo "" >> ${stSMNA}     
     tail -n50 /home/luiz.sapucci/SMNA_v3.0.0.t12717/SMG/run/preOperacao_set_FluxINPE.log >> ${stSMNA}
   fi

   if [ ${ContNotOk} -gt 96 ] && [ ${CODIerroGSI} -eq 3 ] ; then
     echo ""
     echo "######################################################################"   
     echo "Enviando aviso para os responsaveis providenciar os dados disponiveis"
     echo "Tempo de espera: "  $((ContNotOk*5)) "Minutos"
     echo "Que e maior que o tempo normal de 6 depois da hora sinotica processada"
     echo "######################################################################"  
     ...
     echo "Rodada das "$LABELI  >> ${stSMNA}
     echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" >> ${stSMNA}
     echo "Status: NCEP-ATRASADO dados do FTP do NCEP" >> ${stSMNA}
     echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" >> ${stSMNA}
     echo "" >> ${stSMNA}
     echo "     Tempo de espera: "  $((ContNotOk*30)) "Minutos" >> ${stSMNA}
     echo "Que e maior que o tempo normal de 6 depois da hora sinotica processada" >> ${stSMNA}
     echo " Acionar equipe de dados no pre do CPTEC.... "  >> ${stSMNA}
     echo "" >> ${stSMNA}     
     tail -n50 /home/luiz.sapucci/SMNA_v3.0.0.t12717/SMG/run/preOperacao_ago_FluxINPE.log >> ${stSMNA}
   fi
